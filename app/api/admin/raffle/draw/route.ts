...
//
// Body: { n?: number } (defaults to 1 — we just need one address for Henry).
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Empty body is fine — default to a single winner.
  }
  const n =
  ...
  const draw = await drawWinners(n);
  if (draw.entrantCount === 0) {
    return NextResponse.json(
      { error: "No entrants yet — nothing to draw." },
      { status: 409 },
    );
  }
